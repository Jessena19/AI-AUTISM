...
def draw_result_screen(frame, results: dict):
    ...
    att  = results['attention_avg']    # = % of time ATTENTIVE
    dist = results['distraction_avg']  # = % of time DISTRACTED
    a_col = color_for(att)
    d_col = color_for(100 - dist)

    # ── Big score circles ──
    # Attention = % of session time with ATTENTIVE label
    cv2.circle(bg, (FW//4, 175), 90, (30,30,30), -1)
    cv2.circle(bg, (FW//4, 175), 90, a_col, 3)
    a_txt = f"{att:.1f}%"
    (tw,th),_ = cv2.getTextSize(a_txt, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2)
    cv2.putText(bg, a_txt, (FW//4 - tw//2, 175 + th//2),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, a_col, 2, cv2.LINE_AA)
    t("TIME ATTENTIVE", FW//4 - 80, 285, a_col, 0.48, True)
    t(f"avg score {results['att_score_avg']:.0f}/100", FW//4 - 55, 305, (100,100,100), 0.38)

    # Distraction = % of session time with DISTRACTED label
    cv2.circle(bg, (3*FW//4, 175), 90, (30,30,30), -1)
    cv2.circle(bg, (3*FW//4, 175), 90, d_col, 3)
    d_txt = f"{dist:.1f}%"
    (tw,th),_ = cv2.getTextSize(d_txt, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2)
    cv2.putText(bg, d_txt, (3*FW//4 - tw//2, 175 + th//2),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, d_col, 2, cv2.LINE_AA)
    t("TIME DISTRACTED", 3*FW//4 - 85, 285, d_col, 0.48, True)
    t(f"avg score {results['dis_score_avg']:.0f}/100", 3*FW//4 - 55, 305, (100,100,100), 0.38)
...
class TwoMinSession:
    def __init__(self, source=0):
        ...
        self.att_history   = []   # per-frame EMA attention score
        self.dis_history   = []   # per-frame EMA distraction score (tracked separately)
        self.head_history  = []
        self.gaze_history  = []
        self.ear_history   = []
        self.blink_history = []
        self.label_history = []   # "ATTENTIVE" / "DISTRACTED" per frame
    ...
    def _compute_results(self):
        """Compute final averaged results over the 2-minute session."""
        if not self.att_history:
            return None

        att_arr   = np.array(self.att_history,   dtype=float)
        dis_arr   = np.array(self.dis_history,    dtype=float)
        head_arr  = np.array(self.head_history,   dtype=float)
        gaze_arr  = np.array(self.gaze_history,   dtype=float)
        ear_arr   = np.array(self.ear_history,    dtype=float)
        blink_arr = np.array(self.blink_history,  dtype=float)
        lbl_arr   = np.array(self.label_history)

        # ── Core: use label history for true attentive/distracted time
        #    This counts frames where the HYSTERESIS label was DISTRACTED
        #    NOT just score < threshold — this is what the user actually saw
        pct_att = float(np.mean(lbl_arr == "ATTENTIVE") * 100)
        pct_dis = float(np.mean(lbl_arr == "DISTRACTED") * 100)

        # ── Average scores — separately averaged for each state
        att_avg = float(np.mean(att_arr))   # mean attention score across all frames
        dis_avg = float(np.mean(dis_arr))   # mean distraction score across all frames

        # ── The displayed "final" scores should reflect proportion of time,
        #    not just the average score value.
        #    E.g. if you were distracted 60% of time, distraction score = 60
        final_att_score = pct_att    # = % of time label was ATTENTIVE
        final_dis_score = pct_dis    # = % of time label was DISTRACTED

        blink_rate_avg = float(np.mean(blink_arr)) if len(blink_arr) else 0.0

        return dict(
            # These are the BIG numbers shown in circles
            attention_avg   = round(final_att_score, 1),
            distraction_avg = round(final_dis_score, 1),

            # Raw score averages (shown in sub-breakdown)
            att_score_avg   = round(att_avg, 1),
            dis_score_avg   = round(dis_avg, 1),

            # Sub-scores
            head_avg        = round(float(np.mean(head_arr)),  1),
            gaze_avg        = round(float(np.mean(gaze_arr)),  1),
            ear_avg_s       = round(float(np.mean(ear_arr)),   1),
            blink_avg       = round(float(np.mean(blink_arr)), 1),

            # Stats
            total_blinks    = self.blinks,
            blink_rate_avg  = round(blink_rate_avg, 1),
    ...
    def run(self):
        ...
        while True:
            ...
            if results_mp.multi_face_landmarks:
                ...
                self.dis_history.append(round(100 - att_ema, 2))
    ...
    def _show_results(self, results_data):
        ...
        print("═"*54)
        print(f"  Time Attentive     : {results_data['pct_attentive']:.1f}%  of session")
        print(f"  Time Distracted    : {results_data['pct_distracted']:.1f}%  of session")
        print(f"  ─────────────────────────────")
        print(f"  Avg Attention Score: {results_data['att_score_avg']:.1f} / 100")
        print(f"  Avg Distract Score : {results_data['dis_score_avg']:.1f} / 100")
        print(f"  ─────────────────────────────")
